    ...
    { // Traverse operators/nodes in topological order, from the sources to the sinks
      ...
      // the true output rate of the upstream operator(s) to estimate its optimal level of parallelism
        ...
        { // Initially, 'true_output_rates' is empty since the source operators do not have inputs
            ...
            {
                // pub optimal_parallelism_per_epoch: HashMap<Epoch,OperatorInstances>
                ...
                let optimal_instances_per_epoch: f64 = 1.0;
                ...
                graph[idx].output_rates_per_epoch.insert(epoch, input_rate);
                // Estimate the aggregated true output rate of the current operator when configured with the optimal number of instances

                let rate1 = input_rate * estimated_true_output_rate[epoch as usize].1 / true_processing_rate[epoch as usize].1;
                let rate2 = estimated_true_output_rate[epoch as usize].1 * optimal_instances_per_epoch / (op_instances as f64);
                 ...
                 estimated_true_output_rate[epoch as usize].1 = output_rate_new;
        ...
        }
        {
            ...
            for (&epoch, &output_rate) in op_info.output_rates_per_epoch.iter()
            {
                ...
                optimal_config.push_str(&format!("{},{}",op_info.id, output_rate))
            }
        }
    }
    if verbose
